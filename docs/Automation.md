...
| `_TEST_BQ_LOCATION` | The location of BigQuery datasets used in training and prediction pipelines. | `europe-west2` (must match the dataset region; use `US`/`EU` only for multi-region datasets) |
