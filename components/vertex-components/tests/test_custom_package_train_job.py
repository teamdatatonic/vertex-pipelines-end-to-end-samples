...
custom_package_train_job = vertex_components.custom_package_train_job.python_func


@mock.patch("google.cloud.aiplatform.CustomPythonPackageTrainingJob")
@mock.patch("os.path.exists")
@mock.patch("builtins.open", new_callable=mock.mock_open, read_data="{}")
def test_custom_package_train_job(mock_open, mock_exists, mock_job, tmpdir):
    """
    Checks that the custom package training job method is called
    """
    ...
    mock_job.assert_called_once_with(
        display_name="my-job",
        python_package_gcs_uri="gs://my-bucket/package.tar.gz",
        python_module_name="my_module",
